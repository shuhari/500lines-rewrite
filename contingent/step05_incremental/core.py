...
class BuildContext:
    ...
    def get_build_path(self, name, ext):
        return os.path.join(self.build_dir, name + ext)
    ...
    def get_file_timestamp(self, file_path):
        """Get modify time of file, or None if file not exists"""
        if os.path.exists(file_path):
            return datetime.fromtimestamp(os.stat(file_path).st_mtime)
        return None

    def get_src_timestamp(self, filename):
        return self.get_file_timestamp(os.path.join(self.src_dir, filename))

    def get_build_timestamp(self, filename):
        return self.get_file_timestamp(os.path.join(self.build_dir, filename))
